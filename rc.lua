...
-- fancy launcher
local launcher = function()
  awful.spawn("rofi -show combi -theme " .. home(".config/awesome/themes/starenka/rofi.rasi"))
end



-- these tools (from ~/prac/letools, symlinked into ~/bin) are invoked by
-- absolute path rather than bare name: awesome's own process PATH (inherited
-- from sddm, not the interactive shell) doesn't include ~/bin, so a bare
-- name fails to spawn - confirmed via /proc/<awesome-pid>/environ.
